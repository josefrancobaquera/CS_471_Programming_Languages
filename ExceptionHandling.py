 ...
 # Program/File Name: ExceptionHandling.py
 ...
 # exit the program by entering a negative integer only. Furthermore, we assume that 
 # the user will run the program using Python 3 only. 
 # 
 ...
 # the program will check if it's an empty String. If it is, it will use a "continue" 
 # to get another input. If it's not, it will then check if it is an integer. If the 
 # input is not an integer, a ValueError exception is thrown. The program will then print 
 ...
 # throws a AssertionError exception and exits the while loop. If it is not negative, 
...
while ( 1 ):
   ...
   # Get user input.
   new_grade = input( "Enter a Grade Between 0 and 100 (Enter a Negative Integer to Exit): ")
   ...
   try:
      ...
      # If user inputted a negative number, invoke an AssertionError 
      # exception. 
      assert new_grade >= 0
   ...
   # and continue while loop. 
       ...
       print("Error Handler: Oops! You entered a non-integer. Please try again.")
       continue
       
   # If a user inputs a negative number, break from while loop.
   except AssertionError:
      print ("Error Handler: Inputted negative number. Exiting while loop.")
      break
  
   # Handle user input appropriately.  
   except IndexError:
       
      print ("Error Handler: IndexError Exception - Handling user input.")

      # Get the corresponding index of the grade inputted. We note that indexes 
      # start at 0 in Python.
      index = new_grade//10
      ...
      if new_grade > 100:
         print ("Error Handler: New grade ", new_grade, " is out of range.")
...
for index in range(len(freq)):
   ...
   # Print in columns. 
   print ('{:<9}{:<16}{:<9}'.format(limit_1, limit_2, freq[index]))
